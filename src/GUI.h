...
static TwEnumVal Stepper[] = { {DER, "DER"}, {POBD, "POBD"} };

struct GUI
{
    GUI ( ModelParameters* model, double* t ) : m_model(model) {
        m_bar = TwNewBar("Main");
        TwDefine(" Main label='~ Parameters ~' color='100 100 100' alpha=128 fontSize=3 position='10 10' size='270 550' valuesWidth=fit ");
		...
		m_stepper_type = TwDefineEnum("stepper type", Stepper, 2);
		TwAddVarRW(m_bar, "type", m_stepper_type, &(model->m_stepper_type), "group=Stepper");
		...
		TwAddVarRW(m_bar, "friction coefficient", TW_TYPE_DOUBLE, &(model->m_friction), "group=Strands");
